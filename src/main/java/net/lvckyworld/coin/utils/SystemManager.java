...
public class SystemManager {
    ...
    public static void setup() throws SQLException {
        ...
        FileConfiguration cfg = getConfiguration();
        MySQL.connect();
        PreparedStatement ps = MySQL.getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS Coins (UUID VARCHAR(100), Spielername VARCHAR(100), Coins BIGINT)");
        ps.executeUpdate();
        Bukkit.getConsoleSender().sendMessage(MySQL.sqlPrefix + "§aLW-Coins registriert");
    }


    private static File getMySQL() {return new File("plugins/Coins/MySQL.yml");}
